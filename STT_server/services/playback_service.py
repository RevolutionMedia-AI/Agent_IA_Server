...
def emit_playback_item(session: CallSession, item: dict) -> bool:
    ...
    # ponytail: 2026-08-28 — A capture moved out of emit_playback_item.
    # That site only sees 160-byte frames AFTER AudioFrameProcessor,
    # which is useless for the forensic A/B test (it would be a copy
    # of B). Capture A now lives in the TTS adapter right after
    # base64-decode so the diff isolates the framer / queue / pacing.
        ...
        while True:
            ...
            if item_type == "audio":
                ...
                for frame in frame_proc.feed(chunk):
                    ...
                    # send_twilio_media (right before the base64
                    # encode) so the captured bytes are guaranteed
                    # identical to the bytes on the wire. Previously
                    # this site was one frame-constant away from
                    # send_twilio_media — same bytes in practice, but
                    # "in practice" is exactly the kind of assumption
                    # the forensic test is meant to retire.
