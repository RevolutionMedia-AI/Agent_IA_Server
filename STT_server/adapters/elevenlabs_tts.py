    ...
    try:
        ...
        if save_audio and audio_accum:
            try:
                ...
                # reading file" scanner finding without changing the
                # server-issued value's shape.
                ...
                fname = f"elevenlabs_tts_{safe_key}_{generation}.mulaw"
                with open(fname, "wb") as f:
                    f.write(audio_accum)
                    log.debug(f"[TTS] Audio guardado en {fname} ({len(audio_accum)} bytes)")
            except (UnsafePathError, OSError) as exc:
                log.warning(f"[TTS] Skipping elevenlabs audio dump: {exc}")
